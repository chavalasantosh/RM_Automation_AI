...
class AdvancedMatchingEngine:
    ...
    def add_project(self, project: ProjectRequirement) -> None:
        self.projects[project.id] = project

    def get_bench_resources(self) -> List[Dict]:
        return [
            {
                "id": r.id,
                "name": r.name,
                "primary_skills": [{"name": s.name, "level": s.level.value} for s in r.primary_skills],
                "secondary_skills": [{"name": s.name, "level": s.level.value} for s in r.secondary_skills],
                "location": r.location,
                "preferred_work_type": r.preferred_work_type,
                "current_status": r.current_status
            }
            for r in self.resources.values()
            if r.current_status == "BENCH"
        ]
        ...
        return [
            {
                "id": p.id,
                "title": p.title,
                "required_primary_skills": p.required_primary_skills,
                "required_secondary_skills": p.required_secondary_skills,
                "location": p.location,
